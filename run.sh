...
sudo -E ./bin/upf -c ./config/upfcfg.yaml -l ${LOG_PATH}upf.log &
SUDO_UPF_PID=$!
sleep 0.1
...
for NF in ${NF_LIST}; do
    ./bin/${NF} -c ./config/${NF}cfg.yaml -l ${LOG_PATH}${NF}.log &
    PID_LIST+=($!)
    sleep 0.1
done

if [ $N3IWF_ENABLE -ne 0 ]; then
    sudo ./bin/n3iwf -c ./config/n3iwfcfg.yaml -l ${LOG_PATH}n3iwf.log &
    SUDO_N3IWF_PID=$!
    sleep 1
